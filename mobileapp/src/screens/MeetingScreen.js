...
class MeetingScreen extends React.Component {
    ...
    _doCall = () => {
        const { meetingID, password, isCononected } = this.state;

        if (meetingID == '') {
            this.setState({ errorMessage: 'Meeting ID Should not be empty.' });
        } else {
    ...
    render() {
        const { meetingID, width, height, isLoading, errorMessage } = this.state;
                    ...
                    <View style={{paddingBottom:'50%', width: width, height: height, justifyContent: 'center', paddingHorizontal: 30, paddingVertical: 15 }}>
                        ...
                        >
                            <View style={{ alignSelf: 'center', justifyContent: 'center', marginBottom: 40 }}>
                                <Text style={{ color: 'black', fontSize: 30, fontWeight: '700' }}>Your Meeting</Text>
                            </View>
                            <View style={{ paddingVertical: 10 }}>
                                <Text style={{ color: 'black', fontSize: 18, marginBottom: 20 }}>Meeting ID</Text>
                                <TextInput
                            ...
                            <View style={{ paddingVertical: 10, flexDirection: 'row', }}>
                                <TouchableOpacity onPress={() => this._doCall()}
                                    ...
                                    <Text style={{ color: 'white', fontSize: 18, alignSelf: 'center' }}>Join Meeting</Text>
                            ...
                            <View style={{ paddingVertical: 10, flexDirection: 'row', }}>
                                <TouchableOpacity onPress={() => this._doCall()}
                                    style={{ backgroundColor: '#007bff', borderRadius: 5, paddingVertical: 10, width: '100%' }}>
                                    <Text style={{ color: 'white', fontSize: 18, alignSelf: 'center' }}>Create Meeting</Text>
